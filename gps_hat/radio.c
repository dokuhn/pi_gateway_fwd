...
static void write_to_register(uint8_t addr, uint8_t value){
...
static void opmode (uint8_t spidev, uint8_t mode) {
    write_to_register( REG_OPMODE, (read_register( REG_OPMODE) & ~OPMODE_MASK) | mode);
}
...
static void opmodeLora(uint8_t spidev) {
    ...
    write_to_register(REG_OPMODE, u);
}


/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
void setpower(uint8_t spidev, uint8_t pw) {
    write_to_register( REG_PADAC, 0x87);
    if (pw < 5) pw = 5;
    if (pw > 20) pw = 20;
    write_to_register( REG_PACONFIG, (uint16_t)(0x80 | ((pw - 5) & 0x0f)));
}
...
void setfreq(uint8_t spidev, long frequency)
{
    ...
    write_to_register( REG_FRF_MSB, (uint8_t)(frf >> 16));
    write_to_register( REG_FRF_MID, (uint8_t)(frf >> 8));
    write_to_register( REG_FRF_LSB, (uint8_t)(frf >> 0));
}
...
void setsf(uint8_t spidev, int sf)
{
    ...
    if (sf == 6) {
        write_to_register( REG_DETECTION_OPTIMIZE, 0xc5);
        write_to_register( REG_DETECTION_THRESHOLD, 0x0c);
    } else {
        write_to_register( REG_DETECTION_OPTIMIZE, 0xc3);
        write_to_register( REG_DETECTION_THRESHOLD, 0x0a);
    }

    write_to_register( REG_MODEM_CONFIG2, (read_register( REG_MODEM_CONFIG2) & 0x0f) | ((sf << 4) & 0xf0));
    //MSG_LOG(DEBUG_SPI, "SPI", "SF=0x%02x\n", sf, read_register( REG_MODEM_CONFIG2));
}
...
void setsbw(uint8_t spidev, long sbw)
{
    ...
    write_to_register( REG_MODEM_CONFIG, (read_register( REG_MODEM_CONFIG) & 0x0f) | (bw << 4));
}
...
void setcr(uint8_t spidev, int denominator)
{
    ...
    write_to_register( REG_MODEM_CONFIG, (read_register( REG_MODEM_CONFIG) & 0xf1) | (cr << 1));
}
...
{
    write_to_register( REG_PREAMBLE_MSB, (uint8_t)(length >> 8));
    write_to_register( REG_PREAMBLE_LSB, (uint8_t)(length >> 0));
}
...
{
    write_to_register( REG_SYNC_WORD, sw);
}
...
{
    if (nocrc)
        write_to_register( REG_MODEM_CONFIG2, read_register( REG_MODEM_CONFIG2) & 0xfb);
    else
        write_to_register( REG_MODEM_CONFIG2, read_register( REG_MODEM_CONFIG2) | 0x04);
}
...
int lgw_spi_open(int channel) {
    ...
    spidev_fd = wiringPiSPISetup(channel, SPI_SPEED);

    if( spidev_fd >= 0){
        fprintf(stderr, "Note: SPI port opened and configured ok\n");
    }else{
        return -1;
    }
...
void setup_channel(radiodev *dev)
{
    ...
    ASSERT((read_register( REG_OPMODE) & OPMODE_LORA) != 0);
    ...
    write_to_register( REG_LNA, LNA_MAX_GAIN);

    // Auto AGC Low datarate optiomize
    if (dev->sf < 11)
        write_to_register( REG_MODEM_CONFIG3, SX1276_MC3_AGCAUTO);
    else
        write_to_register( REG_MODEM_CONFIG3, SX1276_MC3_LOW_DATA_RATE_OPTIMIZE);

    //500kHz RX optimization
    write_to_register( 0x36, 0x02);
    write_to_register( 0x3a, 0x64);

    // configure output power,RFO pin Output power is limited to +14db
    //write_to_register( REG_PACONFIG, 0x8F);
    //write_to_register( REG_PADAC, read_register( REG_PADAC)|0x07);
}
...
void rxlora(radiodev *dev, uint8_t rxmode)
{
    ...
    if (dev->invertio) {
        write_to_register( REG_INVERTIQ, (read_register( REG_INVERTIQ) & INVERTIQ_RX_MASK & INVERTIQ_TX_MASK) | INVERTIQ_RX_ON | INVERTIQ_TX_OFF);
        write_to_register( REG_INVERTIQ2, INVERTIQ2_ON);
    } else {
        write_to_register( REG_INVERTIQ, (read_register( REG_INVERTIQ) & INVERTIQ_RX_MASK & INVERTIQ_TX_MASK) | INVERTIQ_RX_OFF | INVERTIQ_TX_OFF);
        write_to_register( REG_INVERTIQ2, INVERTIQ2_OFF);
    }


    if (rxmode == RXMODE_RSSI) { // use fixed settings for rssi scan
        write_to_register( REG_MODEM_CONFIG, RXLORA_RXMODE_RSSI_REG_MODEM_CONFIG1);
        write_to_register( REG_MODEM_CONFIG2, RXLORA_RXMODE_RSSI_REG_MODEM_CONFIG2);
    }

    write_to_register( REG_MAX_PAYLOAD_LENGTH, 0x80);
    //write_to_register( REG_PAYLOAD_LENGTH, PAYLOAD_LENGTH);
    write_to_register( REG_HOP_PERIOD, 0xFF);
    write_to_register( REG_FIFO_RX_BASE_AD, 0x00);
    write_to_register( REG_FIFO_ADDR_PTR, 0x00);

    // configure DIO mapping DIO0=RxDone DIO1=RxTout DIO2=NOP
    write_to_register( REG_DIO_MAPPING_1, MAP_DIO0_LORA_RXDONE|MAP_DIO1_LORA_RXTOUT|MAP_DIO2_LORA_NOP);

    // clear all radio IRQ flags
    write_to_register( REG_IRQ_FLAGS, 0xFF);
    // enable required radio IRQs
    write_to_register( REG_IRQ_FLAGS_MASK, ~rxlorairqmask[rxmode]);
    ...
    if (rxmode == RXMODE_SINGLE) { // single rx
        ...
        //write_to_register( REG_OPMODE, OPMODE_RX_SINGLE);
...
bool received(uint8_t spidev, struct pkt_rx_s *pkt_rx) {
    ...
    if ((irqflags & IRQ_LORA_RXDONE_MASK) && (irqflags & IRQ_LORA_CRCERR_MASK) == 0) {
        ...
        uint8_t currentAddr = read_register( REG_FIFO_RX_CURRENT_ADDR);
        uint8_t receivedCount = read_register( REG_RX_NB_BYTES);

        pkt_rx->size = receivedCount;

        write_to_register( REG_FIFO_ADDR_PTR, currentAddr);

        printf("\nRXTX~ Receive(HEX):");
        for(i = 0; i < receivedCount; i++) {
            pkt_rx->payload[i] = (char)read_register( REG_FIFO);
            printf("%02x", pkt_rx->payload[i]);
        }
        printf("\n");

        uint8_t value = read_register( REG_PKT_SNR_VALUE);
        ...
        pkt_rx->rssi = read_register( REG_PKTRSSI) - rssicorr;

        pkt_rx->empty = 0;  /* make sure save the received messaeg */

        return true;
    } /* else if (read_register( REG_OPMODE) != (OPMODE_LORA | OPMODE_RX_SINGLE)) {  //single mode
        write_to_register( REG_FIFO_ADDR_PTR, 0x00);
...
void txlora(radiodev *dev, struct pkt_tx_s *pkt) {
    ...
    write_to_register( REG_PARAMP, 0x08);
    ...
    // configure output power,RFO pin Output power is limited to +14db
    write_to_register( REG_PACONFIG, (uint8_t)(0x80|(15&0xf)));

    if (pkt->invert_pol) {
        write_to_register( REG_INVERTIQ, (read_register( REG_INVERTIQ) & INVERTIQ_RX_MASK & INVERTIQ_TX_MASK) | INVERTIQ_RX_OFF | INVERTIQ_TX_ON);
        write_to_register( REG_INVERTIQ2, INVERTIQ2_ON);
    } else {
        write_to_register( REG_INVERTIQ, (read_register( REG_INVERTIQ) & INVERTIQ_RX_MASK & INVERTIQ_TX_MASK) | INVERTIQ_RX_OFF | INVERTIQ_TX_OFF);
        write_to_register( REG_INVERTIQ2, INVERTIQ2_OFF);
    ...
    write_to_register( REG_DIO_MAPPING_1, MAP_DIO0_LORA_TXDONE|MAP_DIO1_LORA_NOP|MAP_DIO2_LORA_NOP);
    // clear all radio IRQ flags
    write_to_register( REG_IRQ_FLAGS, 0xFF);
    // mask all IRQs but TxDone
    write_to_register( REG_IRQ_FLAGS_MASK, ~IRQ_LORA_TXDONE_MASK);

    // initialize the payload size and address pointers
    write_to_register( REG_FIFO_TX_BASE_AD, 0x00); write_to_register( REG_FIFO_ADDR_PTR, 0x00);

    for (i = 0; i < pkt->size; i++) { 
        write_to_register( REG_FIFO, pkt->payload[i]);
    }

    write_to_register( REG_PAYLOAD_LENGTH, pkt->size);
    ...
    // mask all IRQs
    write_to_register( REG_IRQ_FLAGS_MASK, 0xFF);
...
void single_tx(radiodev *dev, uint8_t *payload, int size) {
    ...
    for (i = 0; i < size; i++) { 
        write_to_register( REG_FIFO, payload[i]);
    }

    write_to_register( REG_PAYLOAD_LENGTH, size);
